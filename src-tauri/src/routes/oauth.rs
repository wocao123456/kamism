...
async fn fetch_oauth_user(state: &AppState, oauth_type: &str, code: &str) -> Result<(String, String, String, Option<String>), String> {
    ...
    let qq_number = find_str(&v, &["qq", "uin", "qq_number", "account"]);
    let email = email.or_else(|| {
        qq_number
            .filter(|q| oauth_type == "qq" && q.chars().all(|c| c.is_ascii_digit()))
            .map(|q| format!("{}@qq.com", q))
    });
    Ok((open_id, username, avatar, email))
}


fn stable_tmp_email(provider: &str, open_id: &str) -> String {
    let hash = EncryptedFieldsOps::generate_hash(&format!("{}:{}", provider, open_id));
    let short = hash.chars().take(12).collect::<String>();
    format!("{}@kamism.com", short)
}

fn oauth_email_or_fallback(provider: &str, open_id: &str, oauth_email: Option<&str>) -> String {
    if let Some(email) = oauth_email.filter(|e| e.contains('@')) {
        return email.to_lowercase();
    }
    if provider == "qq" && open_id.chars().all(|c| c.is_ascii_digit()) {
        return format!("{}@qq.com", open_id);
    }
    stable_tmp_email(provider, open_id)
}
