...
pub async fn start_server() -> anyhow::Result<()> {
    ...
    let app = axum::Router::new()
        ...
        .merge(routes::plan_config::public_plan_router(state.clone()))
        ...
        .merge(routes::system_config::install_public_router())
        .merge(routes::system_update::system_update_router(state.clone()))
        .merge(routes::system_runtime::system_runtime_router(state.clone()))
        .merge(routes::system_config::system_config_router(state.clone()))
        .merge(routes::system_config::merchant_recharge_router(state.clone()))
        .nest("/keys", routes::api_keys::api_keys_router(state.clone()))
        .nest("/ts", routes::api_ts::api_ts_router(state.clone()))
...
async fn scan_and_enqueue(pool: &db::DbPool, channel: &Arc<lapin::Channel>) {
    let expired: Vec<(uuid::Uuid,)> = sqlx::query_as(
        "SELECT id FROM merchants WHERE plan <> 'free' AND plan_expires_at IS NOT NULL AND plan_expires_at <= NOW()",
