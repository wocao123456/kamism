...
git remote set-url origin https://github.com/wocao123456/kamism.git >/dev/null 2>&1 || true

echo "[$(date)] [1/6] fetch latest code..." >> "$LOG"
...
if [ -n "${DATABASE_URL:-}" ]; then
  echo "[$(date)] [5/6] write installed version to database..." >> "$LOG"
  psql "$DATABASE_URL" -c "
