...
export const adminApi = {
  ...
  deletePlanConfig: (id: string) => api.delete(`/admin/plan-configs/${id}`),
  getSystemConfig: () => api.get('/admin/system-config'),
  saveSystemConfig: (key: string, value: any) => api.post('/admin/system-config', { key, value }),
  generateRechargeCodes: (data: any) => api.post('/admin/recharge-codes', data),
  getRechargeCodes: (params?: { status?: string; limit?: number }) => api.get('/admin/recharge-codes', { params }),
  deleteRechargeCode: (id: string) => api.delete(`/admin/recharge-codes/${id}`),
  batchDeleteRechargeCodes: (ids: string[]) => api.post('/admin/recharge-codes/batch-delete', { ids }),
  // 风控
...
export const publicSystemConfig = { get: () => api.get('/system-config/public') };
