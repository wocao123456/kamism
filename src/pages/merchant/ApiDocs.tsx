...
import { appsApi, cardsApi } from '../../lib/api';
...
function endpoints(apiKey: string, appId: string, cardCode: string, keyNames: { sign?: string; enc?: string; dec?: string }): Endpoint[] {
  return [
    { id: 'activate', title: '激活卡密', method: 'POST', path: '/v1/activate', group: '授权接口', auth: 'api_key', description: '客户端首次使用卡密时调用，验证卡密并绑定当前设备。设备已绑定时可直接返回成功。', params: [
      ...
      { name: 'app_id', type: 'string', required: true, description: '应用 ID', example: appId || '<app_id>' },
      { name: 'card_code', type: 'string', required: true, description: '卡密内容', example: cardCode || '<card_code>' },
    ...
    { id: 'sign', title: '签名运算', method: 'POST', path: '/api/ts/sign', group: '算法接口', auth: 'bearer', description: '使用 API 管理中配置的签名代码，对传入参数进行签名运算。', params: [
      { name: 'key_name', type: 'string', required: true, description: '配置名称', example: keyNames.sign || 'sign' },
      { name: 'text', type: 'string', required: true, description: '待签名原文', example: '<text>' },
      { name: 'params', type: 'object', description: '自定义参数', example: '{}' },
    ], response: JSON.stringify({ code: 200, msg: 'success', data: { key_name: keyNames.sign || 'sign', result: '<signature>' } }, null, 2), note: '需要在 Header 中传入 Authorization: Bearer <auth_key>。' },
    { id: 'encrypt', title: '加密文本', method: 'POST', path: '/api/ts/encrypt', group: '算法接口', auth: 'bearer', description: '使用 API 管理中配置的加密代码，对传入文本进行加密。', params: [
      { name: 'key_name', type: 'string', required: true, description: '配置名称', example: keyNames.enc || 'enc' },
      { name: 'text', type: 'string', required: true, description: '待加密文本', example: '<text>' },
      { name: 'params', type: 'object', description: '自定义参数', example: '{}' },
    ], response: JSON.stringify({ code: 200, msg: 'success', data: { key_name: keyNames.enc || 'enc', result: '<encrypted>' } }, null, 2), note: '需要在 Header 中传入 Authorization: Bearer <auth_key>。' },
    { id: 'decrypt', title: '解密文本', method: 'POST', path: '/api/ts/decrypt', group: '算法接口', auth: 'bearer', description: '使用 API 管理中配置的解密代码，对密文进行解密还原。', params: [
      { name: 'key_name', type: 'string', required: true, description: '配置名称', example: keyNames.dec || 'dec' },
      { name: 'text', type: 'string', required: true, description: '待解密密文', example: '<encrypted>' },
      { name: 'params', type: 'object', description: '自定义参数', example: '{}' },
    ], response: JSON.stringify({ code: 200, msg: 'success', data: { key_name: keyNames.dec || 'dec', result: '<decrypted>' } }, null, 2), note: '需要在 Header 中传入 Authorization: Bearer <auth_key>。' },
  ];
}
...
export default function ApiDocs() {
  ...
  const [firstAppId, setFirstAppId] = useState('');
  const [firstCardCode, setFirstCardCode] = useState('');
  const [keyNames, setKeyNames] = useState<{ sign?: string; enc?: string; dec?: string }>({});
  useEffect(() => {
    Promise.all([
      appsApi.list({ page: 1, page_size: 1 }),
      cardsApi.list({ page: 1, page_size: 1 }),
      fetch(`${BASE_URL}/api/keys`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } }).then(r => r.json()).catch(() => ({ data: [] })),
    ]).then(([appRes, cardRes, keysRes]: any[]) => {
      if (appRes?.data?.data?.length) setFirstAppId(appRes.data.data[0].id);
      if (cardRes?.data?.data?.length) setFirstCardCode(cardRes.data.data[0].code);
      const keys: any[] = keysRes?.data || [];
      const kn: { sign?: string; enc?: string; dec?: string } = {};
      // 按 name 包含关键词匹配：优先精确匹配 sign / enc / dec
      const signKey = keys.find((k: any) => k.name?.toLowerCase() === 'sign') || keys.find((k: any) => k.name?.toLowerCase().includes('sign'));
      const encKey = keys.find((k: any) => k.name?.toLowerCase() === 'enc') || keys.find((k: any) => k.name?.toLowerCase().includes('enc')) || keys.find((k: any) => k.name?.toLowerCase().includes('加密'));
      const decKey = keys.find((k: any) => k.name?.toLowerCase() === 'dec') || keys.find((k: any) => k.name?.toLowerCase().includes('dec')) || keys.find((k: any) => k.name?.toLowerCase().includes('解密'));
      if (signKey) kn.sign = signKey.name;
      if (encKey) kn.enc = encKey.name;
      if (decKey) kn.dec = decKey.name;
      setKeyNames(kn);
    }).catch(() => {});
  }, []);
  const all = useMemo(() => endpoints(user?.api_key || '<your_api_key>', firstAppId, firstCardCode, keyNames), [user?.api_key, firstAppId, firstCardCode, keyNames]);
