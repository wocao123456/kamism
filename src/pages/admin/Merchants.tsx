  ...
  return (
    <div className="fade-in">
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: 28, width: 360, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <h3 style={{ fontWeight: 700, fontSize: 16 }}>升级套餐</h3>
            </div>
            <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>为商户 <strong style={{ color: 'var(--text)' }}>{upgradeModal.username}</strong> 选择套餐和有效期</p>
            <div>
              <label style={{ fontSize: 12, color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>套餐</label>
              <select className="input" value={selectedPlan} onChange={e => setSelectedPlan(e.target.value)} style={{ width: '100%' }}>
                {Object.values(planConfigs).filter((p:any) => p.plan !== 'free' && p.is_active !== false).map((p:any) => (
                  <option key={p.plan} value={p.plan}>{p.label} · 月¥{Number(p.price_month || 0).toFixed(2)} / 季¥{Number(p.price_quarter || 0).toFixed(2)} / 年¥{Number(p.price_year || 0).toFixed(2)}</option>
                ))}
              </select>
            </div>
            <div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
        {[{ value: '', label: '全部' }, ...Object.values(planConfigs).map((p:any) => ({ value: p.plan, label: p.label }))].map(tab => (
          <button
            key={tab.value}
          ...
          >
            {tab.value !== 'free' && tab.value !== '' && <Crown size={11} style={{ marginRight: 4, verticalAlign: 'middle' }} />}
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20, flexWrap: 'wrap' }}>
        {Object.values(planConfigs).map((config) => (
          <div key={config.plan} style={{
            ...
            background: config.plan !== 'free' ? 'rgba(124,58,237,0.08)' : 'var(--surface)',
            border: `1px solid ${config.plan !== 'free' ? 'rgba(124,58,237,0.3)' : 'var(--border)'}`,
            fontSize: 13,
          }}>
            {config.plan !== 'free' ? <Crown size={14} color="#a78bfa" /> : <Gift size={14} color="var(--text-muted)" />}
            <span style={{ fontWeight: 600, color: config.plan !== 'free' ? '#a78bfa' : 'var(--text)' }}>
              {config.label}
            </span>
      ...
      <div className="table-wrap admin-merchants-table">
        <table>
          <thead><tr>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={m.id} className="data-enter" style={{ animationDelay: `${idx * 25}ms` }}>
                ...
                <td>{m.email}</td>
                <td><span className="mono" style={{ fontSize: 11, color: 'var(--text-muted)', letterSpacing: '0.5px' }}>{m.api_key}</span></td>
                <td>
                  <span style={{
                    display: 'inline-flex', alignItems: 'center', gap: 4,
                    fontSize: 12, fontWeight: 600, padding: '2px 8px', borderRadius: 999,
                    background: m.plan !== 'free' ? 'rgba(124,58,237,0.15)' : 'rgba(255,255,255,0.06)',
                    color: m.plan !== 'free' ? '#a78bfa' : 'var(--text-muted)',
                    border: `1px solid ${m.plan !== 'free' ? 'rgba(124,58,237,0.35)' : 'var(--border)'}`,
                  }}>
                    {m.plan === 'free' ? <><Gift size={10} /> 免费版</> : <><Crown size={10} /> {(planConfigs as any)[m.plan]?.label || m.plan}</>}
                  </span>
                </td>
                ...
                <td>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button
                      className={`btn btn-sm ${m.plan !== 'free' ? 'btn-ghost' : 'btn-primary'}`}
      ...
      <div className="merchant-mobile-list">
        {loading ? (
          Array.from({ length: Math.min(pageSize, 4) }).map((_, i) => <div key={i} className="card merchant-mobile-card"><span className="skeleton" style={{ width: '65%', height: 18 }} /><span className="skeleton" style={{ width: '85%', height: 14 }} /><span className="skeleton" style={{ width: '100%', height: 36 }} /></div>)
        ) : merchants.length === 0 ? (
          <div className="card"><div className="empty-state-text">暂无商户</div></div>
        ) : merchants.map((m, idx) => (
          <div key={m.id} className="card merchant-mobile-card data-enter" style={{ animationDelay: `${idx * 25}ms` }}>
            <div className="merchant-mobile-head">
              <div>
                <div className="merchant-mobile-name">{m.username}</div>
                <div className="merchant-mobile-email">{m.email}</div>
              </div>
              <span className={`badge badge-${m.status}`}>{m.status === 'active' ? '正常' : '禁用'}</span>
            </div>
            <div className="merchant-mobile-meta">
              <span>API：<b className="mono">{m.api_key}</b></span>
              <span>注册：{new Date(m.created_at).toLocaleDateString('zh-CN')}</span>
              {m.plan !== 'free' && <span>到期：{formatExpiry(m.plan_expires_at, m.plan) ?? <b>—</b>}</span>}
            </div>
            <div className="merchant-mobile-plan">
              {m.plan === 'free' ? <><Gift size={13} /> 免费版</> : <><Crown size={13} /> {(planConfigs as any)[m.plan]?.label || m.plan}</>}
            </div>
            <div className="merchant-mobile-actions">
              <button className={`btn btn-sm ${m.plan !== 'free' ? 'btn-ghost' : 'btn-primary'}`} onClick={() => togglePlan(m.id, m.plan)} disabled={planLoading === m.id}>
                {planLoading === m.id ? '处理中...' : m.plan !== 'free' ? '降为免费' : '升级套餐'}
              </button>
              <button className={`btn btn-sm ${m.status === 'active' ? 'btn-danger' : 'btn-ghost'}`} onClick={() => toggleStatus(m.id, m.status)}>
